# -*- coding: utf-8 -*-
"""
Created on Tue Nov 19 10:03:16 2019
...
def get_rectangles(NUM_IMAGES, WIDTH, HEIGHT):
    imgs = []
    rec_size = (30,30)        
...
def generate_dataset(BATCH_SIZE):
    images = get_rectangles(BATCH_SIZE, 128, 128)
...
#train_images = get_rectangles(10000, 512, 512)
#test_images = get_rectangles(10000, 512, 512)
    ...
    self.inference_net = tf.keras.Sequential(
      [
          ...
          tf.keras.layers.Conv2D(
              filters=128, kernel_size=5, strides=(2, 2), padding="SAME", activation='relu'), # 64x64
          tf.keras.layers.Conv2D(
              filters=256, kernel_size=5, strides=(2, 2), padding="SAME", activation='relu'), # 32x32
          tf.keras.layers.Conv2D(
              filters=256, kernel_size=5, strides=(2, 2), padding="SAME", activation='relu'), # 16x16
          tf.keras.layers.Conv2D(
              filters=512, kernel_size=5, strides=(2, 2), padding="SAME", activation='relu'), # 8x8
    ...
    self.generative_net = tf.keras.Sequential(
        [
          ...
          tf.keras.layers.Conv2DTranspose(
              filters=512, kernel_size=5, strides=(2, 2), padding="SAME", activation='relu'),
          tf.keras.layers.Conv2DTranspose(
